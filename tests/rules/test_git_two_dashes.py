...
@pytest.mark.parametrize(
    "command, output",
    [
        (Command("git add -patch", output("--patch")), "git add --patch"),
        (Command("git checkout -patch", output("--patch")), "git checkout --patch"),
        (Command("git checkout -patch", output("--patch")), "git checkout --patch"),
        (Command("git init -bare", output("--bare")), "git init --bare"),
        (Command("git commit -amend", output("--amend")), "git commit --amend"),
        (Command("git push -tags", output("--tags")), "git push --tags"),
        (
            Command("git rebase -continue", output("--continue")),
            "git rebase --continue",
        ),
