...
from thefuck.rules.docker_login import get_new_command, match
...
"""
    assert match(
        Command("docker build -t artifactory:9090/foo/bar:fdb7c6d .", err_response1)
    )
...
"""
    assert not match(
        Command("docker push artifactory:9090/foo/bar:fdb7c6d", err_response3)
    )
...
def test_get_new_command() -> None:
    ...
    assert (
        get_new_command(Command("docker push artifactory:9090/foo/bar:fdb7c6d", ""))
