...
def usage_tracker(mocker: pytest_mock.MockerFixture) -> pytest_mock.MockType:
    return mocker.patch(
...
@pytest.fixture(autouse=True)
def usage_tracker_io(usage_tracker: pytest_mock.MockType) -> StringIO:
    io = StringIO()
...
@pytest.fixture(autouse=True)
def usage_tracker_exists(usage_tracker: pytest_mock.MockType) -> Callable[[], bool]:
    usage_tracker.return_value.exists.return_value = True
    return usage_tracker.return_value.exists


def _assert_tracker_updated(usage_tracker_io: StringIO, pid: int) -> None:
    usage_tracker_io.seek(0)
    info = json.load(usage_tracker_io)
    assert info["pid"] == pid


def _change_tracker(usage_tracker_io: StringIO, pid: int) -> None:
...
@pytest.fixture(autouse=True)
def shell_pid(mocker: pytest_mock.MockerFixture):
    return mocker.patch(
...
@pytest.fixture(autouse=True)
def shell(mocker: pytest_mock.MockerFixture):
    shell = mocker.patch(
...
@pytest.fixture(autouse=True)
def shell_config(mocker: pytest_mock.MockerFixture):
...
@pytest.fixture(autouse=True)
def logs(mocker: pytest_mock.MockerFixture) -> pytest_mock.MockType:
    return mocker.patch(
...
def test_on_first_run(
    usage_tracker_io: StringIO,
...
def test_on_run_after_other_commands(
    usage_tracker_io: StringIO,
    shell_pid: pytest_mock.MockType,
...
def test_on_first_run_from_current_shell(
    usage_tracker_io: StringIO,
    shell_pid: pytest_mock.MockType,
    shell: pytest_mock.MockType,
    logs: pytest_mock.MockType,
...
def test_when_cant_configure_automatically(
    shell_pid: pytest_mock.MockType,
    shell: pytest_mock.MockType,
    logs: pytest_mock.MockType,
) -> None:
