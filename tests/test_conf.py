...
@pytest.fixture
def load_source(mocker: pytest_mock.MockerFixture) -> pytest_mock.MockType:
    return mocker.patch("thefuck.conf.load_source")


def test_settings_defaults(load_source: pytest_mock.MockType, settings) -> None:
...
def test_from_file(load_source: pytest_mock.MockType, settings) -> None:
...
def test_from_file_with_DEFAULT(
    load_source: pytest_mock.MockType, settings: Settings
) -> None:
    load_source.return_value = Mock(
...
def test_from_env(os_environ: dict[str, Any], settings: Settings) -> None:
    os_environ.update(
        {
...
@pytest.mark.usefixtures("load_source")
def test_from_env_with_DEFAULT(os_environ: dict[str, Any], settings: Settings) -> None:
    os_environ.update({"THEFUCK_RULES": "DEFAULT_RULES:bash:lisp"})
    settings.init()
    assert settings.rules == const.DEFAULT_RULES + ["bash", "lisp"]


def test_settings_from_args(settings: Settings) -> None:
...
def test_ignore_if_exists(settings: Settings) -> None:
...
def test_create_if_doesnt_exists(settings: Settings) -> None:
